...
export class ListThemeComponent implements OnInit, OnDestroy {
  ...
  async onViewEdit(theme: Theme, isEditMode: boolean = false) {
    const { ThemeDetailModalModule } = await import('../theme-detail-modal/theme-detail-modal.module');

    this.store.dispatch(showDialog({
      component: ThemeDetailModalModule.getComponent(),
      config: {
        maxWidth: '85%',
        data: {
          theme: theme,
